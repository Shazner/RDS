...
void Activity_log::write_csv(std::string incident){
    ...
    buffer[19] = '\0';
    std::string csvName = std::string(buffer) + incident + ".csv";
    ...
    for(const auto& entry : entries){
        ...
        csv << std::asctime(std::gmtime(&entry_time)) << separator << entry.get_activity() << separator;
        csv << entry.get_first_name() << separator << entry.get_last_name() << separator;
        csv << entry.get_agency() << separator << entry.get_position() << separator;
        csv << entry.get_id() << separator << entry.get_type() << separator << entry.get_notes() << "\r\n";
    }
    csv.close();
}
